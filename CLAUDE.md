...
- **Traduzioni:** argostranslate in CI (gratis, offline) — IT/ES/DE/FR da EN, con glossario override
...
- **Traduzioni:** `_scripts/translate_content.py` (argostranslate EN→IT/ES/DE/FR, parametrico `--lang`) traduce prodotti e blog, converte i prezzi in EUR via `fx_rates` (config.json, override env `FX_RATE_<CUR>`) e ripunta i link interni `/en/`→`/<lang>/`. Glossario override per i termini tech che argos sbaglia + normalizzazione `$`→`€` nei titoli. `primary_keyword` resta EN. Cache hash in `_data/.translate_cache.json`: cache_key via `.as_posix()` (forward-slash), ritraduce solo se il sorgente EN cambia, quindi i fix manuali ai titoli tradotti sopravvivono finché il sorgente EN resta invariato.
